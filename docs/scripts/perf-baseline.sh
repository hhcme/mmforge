...
#   2  One or more formats: PLACEHOLDER (parser returned empty model), but no ERROR.
#
# When MMFORGE_NO_OCCT_ADVISORY=1, STEP/IGES ERROR downgrades to advisory;
# the script still prints the error but exits 0 with an advisory note.
...
# Priority (worst→best): ERROR > PLACEHOLDER > anything else.
WORST_EXIT=0
HAD_ERROR=0
HAD_PLACEHOLDER=0
HAS_NO_OCCT_ERRORS=0
...
for line in "${RESULT_LINES[@]}"; do
  ...
  case "$STATUS" in
    ERROR)
      HAD_ERROR=1
      # STEP/IGES needing OCCT is a known advisory case
      if [ "$FMT" = "STEP" ] || [ "$FMT" = "IGES" ]; then
        HAS_NO_OCCT_ERRORS=1
      fi
      ;;
...
if [ "$HAD_ERROR" -eq 1 ]; then
  if [ "$ADVISORY_NO_OCCT" = "1" ] && [ "$HAS_NO_OCCT_ERRORS" -eq 1 ] && [ "$HAD_PLACEHOLDER" -eq 0 ]; then
    echo "> NOTE: STEP/IGES ERROR(s) are from no-OCCT environment."
    echo ">       MMFORGE_NO_OCCT_ADVISORY=1 downgrades these errors to advisory."
    echo ">       Known: STEP & IGES require OpenCASCADE to produce geometry."
    FINAL_EXIT=0
  else
    FINAL_EXIT=1
  fi
